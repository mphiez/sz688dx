						...
						<table class="table table-hover table-strips">
							...
							<tbody id="produk">
								<tr id="produk_1">
									...
									<td>
										<select class="form-control chosen-select1" id="ppn_1">
											<option value="0">Pilih Pajak</option>
											<option value="10">PPN</option>
											<option value="15">PPN Dan Service</option>
										</select>
									</td>
								...
								<tr id="produk_2">
									...
									<td>
										<select class="form-control chosen-select1" id="ppn_2">
											<option value="0">Pilih Pajak</option>
											<option value="10">PPN</option>
											<option value="15">PPN Dan Service</option>
										</select>
...
<script>
	...
	$('#submit').submit(function(e){
		...
		for(i=1;i<=counter;i++){
			...
			if($('#tanggal_'+i).val() == '' && ($('#akun_debet_'+i).val() != '') && ($('#debet_'+i).val() != '')){
				alert('Tanggal tidak boleh kosong!');
				return false;
			}else if($('#deskripsi_'+i).val() == '' && ($('#akun_debet_'+i).val() != '') && ($('#debet_'+i).val() != '')){
				alert('Keterangan tidak boleh kosong!');
				return false;
			}else if(($('#akun_debet_'+i).val() == '') && ($('#debet_'+i).val() != '')){
				alert('Jenis Transaksi tidak boleh kosong !');
				return false;
			}else if(($('#debet_'+i).val() == '') && ($('#akun_debet_'+i).val() != '')){
		...
		if(transaksi.length > 0){
			$.ajax({
				url: '<?php echo base_url()?>index.php/pembayaran/save_pembayaran',
				type: "POST",
				data: {
					transaksi 			: transaksi
				},
				success: function(datax) {
					...
					if(datax.code == 0){
						...
						location.replace('<?php echo base_url()?>index.php/pembayaran/input');
	...
	function add_product(){
		...
		$('#produk').append('<tr id="produk_'+num+'">'+
									...
									'</td>'+
									'<td>'+
										'<select class="form-control chosen-select1" id="ppn_'+num+'">'+
											'<option value="0">Pilih Pajak</option>'+
											'<option value="10">PPN</option>'+
											'<option value="15">PPN Dan Service</option>'+
										'</select>'+
