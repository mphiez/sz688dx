						...
						<table class="table table-hover table-strips">
							<thead>
								<tr>
									...
									<th width="22%">Akun Debet</th>
									<th width="10%">Debet</th>
									<th width="22%">Akun Kredit</th>
							...
							<tbody id="produk">
								<tr id="produk_1">
									...
									<td><input type="text" id="debet_1" onkeyup="return change_kredit(1)" class="form-control money"></td>
									<td>
										<select class="form-control chosen-select1" id="akun_kredit_1">
											<option value="">Pilih Akun</option>
											<?php 
											foreach($account_list as $row){
											?>
									...
									<td><input type="text" id="kredit_1" onkeyup="return change_debit(1)" class="form-control money"></td>
								</tr>
								<tr id="produk_2">
									<td>
										<div class="input-group">
									...
									<td><input type="text" id="debet_2" onkeyup="return change_kredit(2)" class="form-control money"></td>
									<td>
										<select class="form-control chosen-select1" id="akun_kredit_2">
											<option value="">Pilih Akun</option>
											<?php 
											foreach($account_list as $row){
											?>
									...
									<td><input type="text" id="kredit_2" onkeyup="return change_debit(2)" class="form-control money"></td>
								</tr>
							</tbody>
							<tfoot>
								<tr>
	...
	$('#submit').submit(function(e){
		...
		for(i=1;i<=counter;i++){
			var temp = {
				...
				debet		:$('#debet_'+i).val(),
				akun_kredit	:$('#akun_kredit_'+i).val(),
				kredit		:$('#kredit_'+i).val(),
			}
			if($('#tanggal_'+i).val() == '' && ($('#akun_debet_'+i).val() != '' || $('#akun_kredit_'+i).val() != '') && ($('#debet_'+i).val() != '' || $('#kredit_'+i).val() != '')){
				alert('Tanggal tidak boleh kosong!');
				return false;
			}else if($('#deskripsi_'+i).val() == '' && ($('#akun_debet_'+i).val() != '' || $('#akun_kredit_'+i).val() != '') && ($('#debet_'+i).val() != '' || $('#kredit_'+i).val() != '')){
				alert('Keterangan tidak boleh kosong!');
				return false;
			}else if(($('#akun_debet_'+i).val() == '' || $('#akun_kredit_'+i).val() == '') && ($('#debet_'+i).val() != '' || $('#kredit_'+i).val() != '')){
				alert('Akun Debet dan Akun Kredit tidak boleh kosong !');
				return false;
			}else if(($('#debet_'+i).val() == '' || $('#kredit_'+i).val() == '') && ($('#akun_debet_'+i).val() != '' || $('#akun_kredit_'+i).val() != '')){
				alert('Debet dan Kredit tidak boleh kosong !');
				return false;
			}else if($('#debet_'+i).val() != $('#kredit_'+i).val()){
				alert('Debet dan Kredit tidak sama !');
				return false;
			}else if($('#deskripsi_'+i).val() != '' && $('#akun_debet_'+i).val() != '' && $('#akun_kredit_'+i).val() != '' && $('#debet_'+i).val() != '' && $('#kredit_'+i).val() != ''){
				transaksi.push(temp);
			}
		...
		if(transaksi.length > 0){
			$.ajax({
				url: '<?php echo base_url()?>index.php/transaksi/save_jurnal',
				type: "POST",
				data: {
					transaksi 			: transaksi
				},
				success: function(datax) {
					...
					if(datax.code == 0){
						...
						location.replace('<?php echo base_url()?>index.php/transaksi/jurnal');
	...
	function add_product(){
		...
		$('#produk').append('<tr id="produk_'+num+'">'+
									...
									'<td>'+
										...
										'</select>'+
									'</td>'+
									'<td><input type="text" id="kredit_'+num+'" onkeyup="return change_debit('+num+')" class="form-control money"></td>'+
								'</tr>');
		$('.chosen-select1').chosen({no_results_text: "Oops, nothing found!"});
		$('.date').datepicker({
			format: "yyyy-mm-dd",
